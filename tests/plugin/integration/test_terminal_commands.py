...
from techtree_hermes.host.commands import build_cli_verbs
...
def test_doctor_runs_techtrees_own_output(
    ...
    code = build_cli_verbs(services)["doctor"].handler(_namespace())
...
def test_human_mode_never_adds_the_machine_flags(
    ...
    build_cli_verbs(services)["result"].handler(_namespace(run_id=RUN_ID))
...
def test_watch_follows_the_run_in_the_terminal(
    ...
    build_cli_verbs(services)["watch"].handler(_namespace(run_id=RUN_ID))
...
def test_verify_passes_the_path_through(
    ...
    build_cli_verbs(services)["verify"].handler(_namespace(target="/tmp/proof"))
...
def test_the_exit_code_is_the_cli_s_own(
    ...
    code = build_cli_verbs(services)["status"].handler(_namespace(run_id="fail"))
...
def test_the_demo_command_prints_plain_text(
    ...
    code = build_cli_verbs(services)["demo"].handler(_namespace())
