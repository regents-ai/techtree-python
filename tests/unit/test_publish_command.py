"""What ``techtree proof publish`` puts in front of a person. Decisions 0038.
...
def test_a_machine_that_cannot_be_asked_is_told_which_flag_to_pass(
    ...
    result = invoke(home, "--json", "proof", "publish", PROOF_RUN_ID)
...
def test_saying_no_at_the_prompt_sends_nothing(home: Path) -> None:
    result = invoke(home, "proof", "publish", PROOF_RUN_ID, stdin="n\nn\n")
...
def test_a_proof_that_does_not_verify_is_refused_before_anything_is_asked(
    ...
    result = invoke(home, "--json", "proof", "publish", PROOF_RUN_ID, "--yes")

    assert result.exit_code == EXIT_VERIFICATION
    assert SENT == []


def test_a_build_with_no_run_log_configured_refuses(
    ...
    assert result.exit_code != EXIT_OK
    assert "publication_endpoint_not_configured" in result.stdout
    assert SENT == []
...
def test_a_verified_proof_offers_publishing(home: Path) -> None:
    ...
    assert offer["cli"] == ["techtree", "proof", "publish", PROOF_RUN_ID]
...
def test_the_address_question_is_asked_once_and_defaults_to_no(
    ...
    result = invoke(home, "proof", "publish", PROOF_RUN_ID, stdin="\ny\n")
...
def test_a_mistyped_address_stops_the_publication(home: Path) -> None:
    ...
    result = invoke(
        home, "--json", "proof", "publish", PROOF_RUN_ID, "--yes", "--address", wrong
    )
