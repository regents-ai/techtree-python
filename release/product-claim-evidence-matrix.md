...
One row per public product claim. Each row names the modules that implement it, tests that exist in the trees today (every test name below was located by search before it was written down), the live evidence from the certification programme, and the limitation that must travel with the claim in public copy.

**Where live evidence lives.** Run trees are not committed to this repository — `runs/` is empty by design and raw subject traces stay on the participant's machine because they carry the taskset's expected answers (spec §6.19). Live evidence is therefore cited from the Gate-1 packet (`release/founder-skill-approval-draft.md`, digest recomputed here as `sha256:b3ea3ba12af27af878fd4061f4a115b35f772b7c2ca61e2d4fb82d4b5701da35`, matching the founder's approval phrase in `release/founder-approvals/gate1-founder-skills.md`) and from the committed artifacts this ticket re-hashed itself. Rows where that leaves a real hole carry a Gaps block, and every gap is repeated in the gap register at the end.
...
## What this ticket verified

Every citation below was checked mechanically before it was written down: 102 implementation paths resolve to files that exist, and 255 test citations resolve to a test function that exists in the named file.

Executed here:

- techtree-python `uv run pytest tests/unit` — 2376 passed, 1 skipped (tests/unit/test_skill_scanner.py::test_the_case_collision_rule_holds_on_a_case_folding_filesystem skips on this machine's case-folding filesystem).
- techtree-python `uv run pytest tests/contract` plus every cited integration file in the default selection — 418 passed.
- techtree-python full integration-marked suite (`-m "integration and not real_model"`) — 284 passed.
- The five cited preflight tests — all passed.
- techtree-plugin cited suites (test_approvals, test_guards, test_bootstrap, test_release, test_one_generation_request, test_plugin_doctor, test_one_turn_revision) — 194 passed.

Not executed here:

- The four techtree-ash citations (`test/techtree_web/router_test.exs`, `test/techtree_web/endpoint_test.exs`). The Ash test setup requires a PostgreSQL database that is not available in this environment, so those four test names were verified by reading the files, not by running them.
- No `real_model` test is cited anywhere in this matrix, and none was run — those calls cost money and provision Docker.

Observed in passing, reported not fixed:

- The preflight citations (`tests/preflight/…`) are marked `preflight` and are excluded from the default pytest selection; they run via `make verifiers-preflight` and need a pinned Verifiers install.
- Two preflight tests that this matrix does not cite fail in this environment: `tests/preflight/test_subject_image_pin.py::test_the_tag_still_resolves_to_the_pinned_index` (needs registry network access) and `tests/preflight/test_verifiers_eval_contract.py::test_a_subject_seat_is_refused_by_an_environment_without_one` (the preflight fixture taskset is not installed here). Both look environment-dependent rather than product defects. Recorded here because it was seen, not diagnosed and not fixed.

## Canonical certification runs (Gate-1 §4)

| Run | ID | Score | Report | Bundle | Cost (USD) |
|---|---|---|---|---|---|
| starter #1 (insertion) | `run_d94094506aec482aa7ed35bad011486f` | 0/36 -> 23/36 | `sha256:47253518...` | `sha256:19493177...` | 0.1780 |
| starter #2 (insertion, stability pair) | `run_a6c608b910324d6ca84062dc0c4960c2` | 0/36 -> 23/36 | `sha256:784614a1...` | `sha256:faa8260e...` | 0.1709 |
| engine reference | `run_6ff833ca941745bcae620df4c6c0dc27` | 0/36 -> 36/36 | `sha256:c1a88d20...` | `sha256:d32c7dca...` | 0.1827 |
| rehearsal source | `run_9f2a5025bba849e5b8e8e0b15db855c6` | 0/36 -> 24/36 | `sha256:5668a0f1...` | `sha256:c7b76024...` | 0.1702 |
| v1 vs v2 (replacement) | `run_e29f178181bb4a65b0c518d34f72cd50` | 23/36 -> 24/36 (1 win / 0 losses / 35 ties) | `sha256:421f1a0b...` | `sha256:4db83924...` | 0.0551 |

All five are complete, controlled-with-warnings, score valid, evidence complete, accepted, P1, `executor_kind` verifiers, signed ComparisonExecutionRecord, proof verifies offline, zero cap-killed episodes.

## Coordinates re-verified by this ticket

Recomputed from the working tree with `shasum -a 256`; every value matched the Gate-1 packet.

| Object | Digest (prefix) | Source |
|---|---|---|
| campaign | `sha256:5aef3fb7c16c39b9...` | src/techtree/resources/catalog/campaigns/hello-world-climb.json |
| climb | `sha256:61a7dd46cba4d8ba...` | src/techtree/resources/catalog/climbs/hello-world-climb.json |
...
| engine | `sha256:874cbae03d39358297685e5…` | release/release-core.json engine_digest |
| catalog | `sha256:468e8ab16dc872cd2cef3f1…` | release/release-core.json catalog_digest |
| starter skill file | `sha256:2aff27070177d9f37b99d5b…` | release/skills/hello-world-starter-v1/SKILL.md |
| starter skill tree | `sha256:596d1368ac157975accce7c…` | release/release-core.json starter_skill_digest (the pin, decision 0008) |
| skill improver file | `sha256:e6bc16c4d6740a0c3528c70…` | techtree-plugin/skills/skill-improver/SKILL.md |
| release core | `sha256:80807821d3d5b8bcf0a5223…` | byte-identical in techtree-python/release/release-core.json and techtree-plugin/release-core.json; both recomputed here |
| gate1 packet | `sha256:b3ea3ba12af27af878fd406…` | release/founder-skill-approval-draft.md |
...
- Campaign hello-world-climb sha256:5aef3fb7c16c39b9... — recomputed in this ticket from src/techtree/resources/catalog/campaigns/hello-world-climb.json and equal to the Gate-1 §1/§8 value.
- All five canonical runs anchor on it (Gate-1 §4): the four insertion runs through derived campaign b9e3f00c..., the replacement run through derived campaign f2f04ae5....
- Gate-1 §3: no run-owned or proof-owned file of any completed run was modified after that run finished (method: last journal event vs every file mtime, re-run over the full final run set).

**Limitation.** 'Campaign' is an internal protocol concept; a participant never names one. Immutability is enforced by recomputing digests over frozen canonical bytes — there is no external registry, notary or timestamping authority, so the guarantee is 'this object is not the object it claims to be if a byte moved', not 'this object existed before that moment'.
...
- Membership digest sha256:56f697fb... (Gate-1 §2 and §8), unchanged by the 512->4096 output-cap change and by re-freeze v3.
- Validation receipt sha256:080895d53a967d63... recomputed here; it commits to the same lock.
- All five canonical runs (Gate-1 §4) carry 36 committed tasks per variant, scored in committed order; the v1-vs-v2 run reports 1 win / 0 losses / 35 ties over exactly those 36.

**Limitation.** Fixed membership: v0.1 ships one 36-task selection taken from the head of the frozen dataset order. There is no sampling, no task selection, no held-out split and no rotation. What is proved is that both arms saw identical tasks — not that those tasks are representative of anything.
...
- Validation receipt sha256:080895d53a967d63... and validation evidence sha256:9c4959d3b9217ab8... — both recomputed in this ticket from the shipped catalog and equal to the Gate-1 §1 coordinates.
- Campaign sha256:5aef3fb7c16c39b9... commits to that receipt; tests/integration/test_taskset_validation.py::test_the_local_receipt_is_the_published_receipt binds the shipped bytes to the published ones.

**Limitation.** Mechanical only. The receipt says each task's gold answer scores and a planted wrong answer does not. It says nothing about whether a task is well-posed, unambiguous, non-trivial, or free of leakage into the subject model's training data.
...
- Every insertion run scored the baseline 0/36: run_d94094506aec482aa7ed35bad011486f, run_a6c608b910324d6ca84062dc0c4960c2, run_6ff833ca941745bcae620df4c6c0dc27, run_9f2a5025bba849e5b8e8e0b15db855c6 (Gate-1 §4).
- The committed recorded-evidence fixture tests/fixtures/receipts/recorded/ (baseline and candidate directories, 36 episodes each) is real evidence from run_6ce5e56a7dd341bca8bc6de1d6a60027 and is the input to tests/unit/test_observed_configuration.py.
...
- tests/fixtures/receipts/recorded/provenance.json — real recorded evidence from run_6ce5e56a7dd341bca8bc6de1d6a60027: qwen/qwen3.7-flash, subject_platform linux/arm64, real Docker runtimes, 36 episodes per variant, per-variant raw-trace digests (baseline sha256:6f1e54fe..., candidate sha256:24d7b774...), engine sha256:874cbae0....
- Gate-1 §4: all five canonical runs are graded controlled_with_warnings with zero cap-killed episodes; both variants of each run resolved to the same platform-specific image digest (decision 0007 R5 made that release-blocking).
...
- No trace-level artifact from any of the five canonical runs is committed to this repository. The only committed trace-level evidence is the sanitized recorded-evidence fixture from run_6ce5e56a7dd341bca8bc6de1d6a60027, which Gate-1 §4 classifies as DIAGNOSTIC, not canonical.
...
- `src/techtree/receipts/compare.py` — the observed tool-surface comparison, including the single named SKILL_INDEX_TOOL exception
...
- UpliftReport digests, one per canonical run (Gate-1 §4): sha256:47253518..., sha256:784614a1..., sha256:c1a88d20..., sha256:5668a0f1..., sha256:421f1a0b....
- All five are graded controlled_with_warnings, score valid, evidence complete, accepted, P1.
...
- Proof bundles, one per canonical run (Gate-1 §4): sha256:19493177..., sha256:faa8260e..., sha256:d32c7dca..., sha256:c7b76024..., sha256:4db83924... — each carries 72 signed episode receipts (36 per variant).
- tests/fixtures/receipts/recorded/ — 36 baseline and 36 candidate episodes from run_6ce5e56a7dd341bca8bc6de1d6a60027, committed as the real input the receipt pipeline runs over.
...
- Report digests per canonical run (Gate-1 §4) and the executor public key inside each bundle.
- Gate-1 §8: proof verification result verified for all five canonical runs and independently re-confirmed by the chief from the stored bytes at packet-finalization time.
- Gate-1 packet digest sha256:b3ea3ba12af27af878fd4061f4a115b35f772b7c2ca61e2d4fb82d4b5701da35 — recomputed here and equal to the digest in the founder's Gate-1 approval phrase (release/founder-approvals/gate1-founder-skills.md).
...
- Per-run costs (Gate-1 §4): USD 0.1780, 0.1709, 0.1827, 0.1702, 0.0551 for the five canonical runs; each run carries a signed ComparisonExecutionRecord inside its bundle.
- Gate-1 §10 spend ledger: total USD 1.9717 of the 3.00 programme cap, provider-reported where available.
- tests/fixtures/receipts/recorded/{baseline,candidate}/execution.json — real per-variant execution records from run_6ce5e56a7dd341bca8bc6de1d6a60027.

**Limitation.** Provider revision unavailable. Cost and timing are attributed to a model alias, not to a pinned immutable provider revision, because the provider does not expose one (accepted v0.1 warning model_revision_unavailable, decision 0007 R5). Cost provenance is explicit and may be 'unavailable'; Gate-1 §10 discloses one estimated ledger entry (rehearsal attempt 3, ~USD 0.0010, bounded 0.0023) rather than smoothing it.
...
- Data policy sha256:6c532a43d595286a... — recomputed here from src/techtree/resources/catalog/data-policies/hello-world-climb.json and equal to the Gate-1 §1 value; campaign sha256:5aef3fb7c16c39b9... commits to it.
- Gate-1 §9: no upload path observed (push=false in every resolved config); the committed evidence fixture passes all nine sanitization checks.
...
- Gate-1 §4: 'proof verifies offline' for all five canonical runs; Gate-1 §8: independently re-confirmed by the chief from the stored bytes at packet-finalization time.
- Bundle digests per run (Gate-1 §4): sha256:19493177..., sha256:faa8260e..., sha256:d32c7dca..., sha256:c7b76024..., sha256:4db83924....
...
- The contract asks for a command transcript as live evidence. No `techtree proof verify` transcript is committed in this repository: the run trees the five canonical bundles live in are outside the repo (runs/ is empty by design) and the terminal-journey transcript is an output of WP11e (docs/release/contracts/wp11e.md), which has not run. Current evidence is the Gate-1 §4/§8 attestation, not a citable transcript artifact.
...
- `techtree-plugin/approvals.py` — the host-agent approval surface (read-only evidence)
- `techtree-plugin/tools/run.py`, `techtree-plugin/services/proposal.py` — tools declared as human-confirmed

**Automated test**

- `techtree-plugin/tests/unit/test_approvals.py::test_the_plugin_issues_no_approval_of_its_own`
- `techtree-plugin/tests/unit/test_approvals.py::test_a_forged_confirmation_field_is_simply_ignored`
- `techtree-plugin/tests/unit/test_approvals.py::test_no_confirmation_indicator_is_invented`
- `techtree-plugin/tests/unit/test_approvals.py::test_a_documented_indicator_that_says_no_stops_the_call`
- `techtree-plugin/tests/unit/test_approvals.py::test_the_run_records_the_surface_the_person_actually_answered_on`
- `techtree-plugin/tests/unit/test_approvals.py::test_the_disclosure_says_every_thing_it_has_to_say`
- `techtree-plugin/tests/unit/test_approvals.py::test_the_disclosure_never_promises_a_result`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_plugin_mints_no_approval_of_its_own`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_a_proposal_prepares_a_comparison_and_starts_nothing`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_tool_declares_itself_as_one_a_human_must_confirm`
...
- Gate-1 §9: the native approval boundary sends nothing before consent — the approval surface is rendered from the plugin's own tool schema and, before dispatch, there were 0 host calls, 0 outbound requests and 0 CLI reads; re-verified at the 32,768-token reattempt.
- Gate-1 §7c: before dispatch of the canonical rehearsal, 0 host calls / 0 outbound / 0 CLI reads, and the request digest sha256:12e1aa49... computed BEFORE the call matched the request actually sent, exactly.
- Each canonical run records the approving actor and the surface the answer was given on in its own run journal.

**Limitation.** Surface attestation. The run records which surface the answer was given on and which actor gave it, as attested by that surface. Nothing cryptographically binds a human being to the approval, and Techtree cannot tell an operator typing 'yes' from anything else that reaches that surface.

**Gaps (findings, not fixed here)**

- The approval audit events of the five canonical runs are in run journals under the participant's TECHTREE_HOME, not committed to this repository (runs/ is empty by design). The citable live evidence is the Gate-1 §7c/§9 attestation rather than the journal bytes.
...
- Four failed runs preserved as failed with no Uplift receipt (Gate-1 §4 DIAGNOSTIC): run_a8da75d4... (output-cap), run_af675e08... (closed stream + upstream 429), run_8f46f6c9... (output-cap x2), run_28261de5... (infrastructure_failure_no_measurement, USD 0.0000).
- Gate-1 §3: the failed launcher run is preserved exactly as written — failed state, honest error, no rewriting and no relabeling.
- Gate-1 §6: USD 0.5037 of pre-change spend bought no comparison; that is recorded rather than hidden.

**Limitation.** No Uplift receipt. A failed run yields no report and no proof bundle. The participant gets an honest failure and a preserved run record, but the provider spend already incurred is not recovered.

**Gaps (findings, not fixed here)**

- The four failed run records are preserved on the certification machine, not committed here. The citable evidence is the Gate-1 §4 DIAGNOSTIC listing and §3 immutability method, not the run bytes.
...
- run_e29f178181bb4a65b0c518d34f72cd50 — the replacement run on derived campaign f2f04ae5..., report sha256:421f1a0b..., bundle sha256:4db83924..., 23/36 -> 24/36, 1 win / 0 losses / 35 ties, USD 0.0551 (Gate-1 §4).
- Parent Skill v1 tree sha256:596d1368... versus candidate Skill v2 root sha256:b143866e62bae51a140c5d7374a3a75562489c93d5df7144b6ba65b22b622986 (Gate-1 §8); the v1-vs-v2 records carry per-file digests (Gate-1 §7f).
...
**Public wording.** The guided Hello World revision revises the single SKILL.md. The flow is certified end to end: proposed, scanned, diffed, explicitly approved, evaluated and reported honestly. No copy states or implies that guided revision produced measured uplift.
...
- `techtree-plugin/tests/contract/test_one_generation_request.py::test_the_guided_proposal_makes_exactly_one_request`
- `techtree-plugin/tests/contract/test_one_generation_request.py::test_a_second_ask_is_refused_before_it_reaches_the_provider`
- `techtree-plugin/tests/contract/test_one_generation_request.py::test_an_unusable_answer_is_not_repaired_by_a_second_request`
- `techtree-plugin/tests/contract/test_one_generation_request.py::test_a_transport_failure_is_counted_and_not_retried`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_a_second_proposal_is_refused`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_an_unusable_proposal_still_uses_the_turn`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_diff_is_shown_with_the_policy_and_the_estimate`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_second_run_starts_once_the_diff_and_policy_were_shown`
- `techtree-plugin/tests/unit/test_guards.py::test_a_whole_revised_skill_passes`
- `techtree-plugin/tests/unit/test_guards.py::test_a_patch_is_not_a_revision`
- `techtree-plugin/tests/unit/test_guards.py::test_a_revised_skill_quoting_one_member_is_refused`
- `techtree-plugin/tests/unit/test_guards.py::test_prose_listing_three_distinct_members_is_refused`
...
- Gate-1 §7c: the single authorized reattempt at the predeclared 32,768 ceiling — response_id a2b2c61f2a1fb1b4-SJC, request sha256:12e1aa49... (computed before the call and matching the request actually sent), response sha256:69d0657e..., finish stop, 5,139 in / 7,400 out, USD 0.0417.
- It produced Skill v2 hello-world-v2, root sha256:b143866e62bae51a140c5d7374a3a75562489c93d5df7144b6ba65b22b622986, SKILL.md sha256:7cc03c89..., 1,907 bytes, 47 lines, parent 596d1368... (Gate-1 §8).
- Evaluated exactly once, in run_e29f178181bb4a65b0c518d34f72cd50: 23/36 -> 24/36, 1 win / 0 losses / 35 ties (Gate-1 §4, §7c).
- Gate-1 §7f: ndq.3.42 keeps the two multi-file items deferred — skills/starter.py _stage_document fetches only a single SKILL.md, and uplift/source.py exposes only entrypoint_text to the host.

**Limitation.** The flow is certified; the improvement is not. The +1 is inside v1's own run-to-run noise — v1 measured 23 and 24 across six legs — so no measured-uplift claim is made or permitted. The 0013 §4 demo target (>=32/36, >=6 task uplift) was not met; 0013 already designates that a calibrated aim rather than a guarantee. Release copy states the calibrated 20-27/36 band, never an exact score. In v0.1 the improver can only edit the entrypoint, so auxiliary files are always inherited byte-identical from the parent Skill; no copy may claim the guided improver revises multi-file bundles.
...
- `techtree-plugin/tests/unit/test_bootstrap.py::test_a_missing_cli_produces_one_exact_plan`
- `techtree-plugin/tests/unit/test_bootstrap.py::test_a_plan_is_single_use`
- `techtree-plugin/tests/unit/test_bootstrap.py::test_installation_goes_through_the_hosts_own_terminal_approval`
- `techtree-plugin/tests/unit/test_bootstrap.py::test_an_installed_release_that_matches_verifies`
- `techtree-plugin/tests/unit/test_bootstrap.py::test_an_installed_release_that_differs_is_rejected`
- `techtree-plugin/tests/unit/test_bootstrap.py::test_a_placeholder_release_refuses_the_public_install_flow`
- `techtree-plugin/tests/unit/test_release.py::test_a_rewritten_release_file_is_refused`
- `techtree-plugin/tests/contract/test_plugin_doctor.py::test_this_build_passes_its_own_doctor`
- `techtree-plugin/tests/contract/test_plugin_doctor.py::test_the_runtime_imports_only_the_standard_library`
...
- `tests/unit/test_release_bootstrap.py::test_a_real_wrapper_over_a_placeholder_release_is_refused`
...
- `tests/contract/test_release_artifacts.py::test_this_build_carries_a_self_declaring_placeholder_release`
...
- ReleaseCore sha256:80807821d3d5b8bcf0a5223c96684b04546b250ff677b3e3bb65f768bde18687 — recomputed in this ticket and byte-identical in techtree-python/release/release-core.json and techtree-plugin/release-core.json.
- Frozen lineage (Gate-1 header): plugin 1ce5d4e12910d569a5d968eaac1742a0ed0cb40f, techtree-python 1ad6ecf.

**Limitation.** placeholder_release is still true. cli_source_commit, cli_version, maximum_tested_host_hermes_version, release_id and starter_skill_object_url are placeholders (release/release-core.json placeholder_fields), and the plugin refuses the public install flow while they are. The public install path is certified by WP11b/c/d/e, none of which has run. A Prime or provider account, an API credential and network access are still needed for inference, installation and image retrieval — only a Techtree account is not.

**Gaps (findings, not fixed here)**

- No fresh-install transcript is committed; that artifact is a WP11b/WP11e output, and neither ticket has run. Nothing here demonstrates the public install path end to end on a clean machine.
...
- `techtree-plugin/tests/unit/test_guards.py::test_a_narrative_may_not_state_a_number`
- `techtree-plugin/tests/unit/test_guards.py::test_a_narrative_claiming_a_different_score_is_refused`
- `techtree-plugin/tests/unit/test_guards.py::test_a_narrative_may_not_claim_what_is_not_true_of_a_local_run`
- `techtree-plugin/tests/unit/test_guards.py::test_saying_it_was_not_reproduced_is_fine`
...
- Gate-1 §7h: verified in the frozen build that the proof grade is read from the verified report (presentation/build.py) and never hardcoded into comparison labels; the result display attaches the attestation caveat ('integrity-bound, participant-attested local execution') and a weaker-attestation warning when verification is absent.
- Gate-1 §4 closing rule: release language is 'calibrated to the 20-27/36 band' — never an exact score.

**Limitation.** The explanation Skill is guarded, not verified. The guards refuse numbers, digests, commands, escape codes, credentials and any claim untrue of a local run; they cannot establish that the remaining prose is a good explanation. The subject model is a pinned model the Campaign chooses (qwen/qwen3.7-flash), not the reader's own model, and it is distinct from the host operator model that writes the narrative.
...
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_diff_is_shown_with_the_policy_and_the_estimate`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_plugin_keeps_no_copy_of_the_proposed_skill`
- `techtree-plugin/tests/integration/test_one_turn_revision.py::test_the_approved_call_does_exactly_what_was_described`

**Live evidence**

- Gate-1 §7c: the proposal passed every guard and produced valid Skill v2 hello-world-v2; the sequence was scan -> snapshot -> deterministic diff -> reviewed approval -> one v1-vs-v2 comparison -> second signed receipt.
- Gate-1 §9: the copied-case guard passes with all task inputs supplied, and the structure guard refuses a newline-free file truthfully.

**Limitation.** Mechanical scan only. The scanner checks structure, media types, size, hidden and symlinked paths, credential shapes and copied task material. It does not judge meaning: Gate-1 §7c records that the certified proposal inverted the causal direction of the defect and the scan correctly did not object, because that is a measurement question, not a policy one. Decision 0023 §5 rules out an LLM-based semantic Skill scanner in v0.1, and WP11g records the Skill-conflict-scan limitation.
...
- `techtree-ash/lib/techtree_web/method_surface.ex` — the website's read-only method surface (read-only evidence)
...
- `techtree-plugin/tests/contract/test_plugin_doctor.py::test_the_runtime_cannot_open_a_connection`
- `techtree-plugin/tests/contract/test_plugin_doctor.py::test_no_relay_dependency_exists`

**Live evidence**

- Gate-1 §9: no upload path observed — push=false in every resolved configuration across the certification programme; the committed evidence fixture passes all nine sanitization checks; the detached launcher's environment scrubbing refuses to inherit shell credentials.
- tests/fixtures/receipts/recorded/provenance.json records that raw traces stay local by design (spec §6.19) and are not committed.

**Limitation.** The claim is about the Techtree website and the Verifiers platform, not about the network. Model inference goes to the selected provider under that provider's policies, and installation and container image retrieval need network access. The three-method no-upload proof (static route audit, instrumented application-level HTTP method logging, E2E destination capture) is WP11g and has not been executed.

**Gaps (findings, not fixed here)**

- WP11g (docs/release/contracts/wp11g.md) has not run, so the no-upload evidence today is the Gate-1 §9 observational attestation plus the tests above — not the three-method proof the release requires.

## Summary table

| # | Claim | Implementation (lead module) | Automated test (lead) | Live evidence (lead) | Limitation (short) | Gap |
|---|---|---|---|---|---|---|
| claim-01 | Campaign immutable | `src/techtree/models/base.py` | `tests/unit/test_canonical.py::test_protocol_models_are_frozen` | Campaign hello-world-climb sha256:5aef3fb7c16c39b9 | 'Campaign' is an internal protocol concept; a participant never names one | no |
| claim-02 | Same tasks on both sides | `src/techtree/models/validation.py` | `tests/unit/test_taskset_membership_logic.py::test_the_membership_digest_is_the_digest_of_the_named_ordered_object` | Membership digest sha256:56f697fb | Fixed membership: v0.1 ships one 36-task selection taken from the head of the frozen dataset order | no |
| claim-03 | Taskset validated before it is used | `src/techtree/models/validation.py` | `tests/integration/test_taskset_validation.py::test_every_task_passes_gold_and_setup` | Validation receipt sha256:080895d53a967d63 | Mechanical only | no |
| claim-04 | Neutral baseline | `src/techtree/manifests/builder.py` | `tests/unit/test_manifest_builder.py::test_the_baseline_preserves_the_campaign_and_carries_no_skill` | Every insertion run scored the baseline 0/36: run_d94094506aec482aa7ed35bad011486f, run_a6c608b9103… | Toy task | no |
| claim-05 | Clean subjects | `src/techtree/verifiers/config.py` | `tests/unit/test_verifiers_config.py::test_a_runtime_other_than_docker_cannot_be_named` | tests/fixtures/receipts/recorded/provenance.json — real recorded evidence from run_6ce5e56a7dd341bc… | Local executor | yes |
| claim-06 | Only the Skill changed | `src/techtree/manifests/compare.py` | `tests/unit/test_observed_comparison.py::test_an_unauthorized_observed_difference_is_invalid` | UpliftReport digests, one per canonical run (Gate-1 §4): sha256:47253518..., sha256:784614a1..., sh… | Known derived description delta, plus one accepted warning | no |
| claim-07 | Per-task receipts | `src/techtree/receipts/episode.py` | `tests/unit/test_episode_receipt_builder.py::test_every_committed_task_gets_exactly_one_receipt` | Proof bundles, one per canonical run (Gate-1 §4): sha256:19493177..., sha256:faa8260e..., sha256:d3… | Internal evidence | no |
| claim-08 | Signed report | `src/techtree/crypto.py` | `tests/unit/test_crypto.py::test_sign_and_verify_round_trip` | Report digests per canonical run (Gate-1 §4) and the executor public key inside each bundle | Participant-attested | no |
| claim-09 | Cost and timing are recorded honestly | `src/techtree/receipts/execution.py` | `tests/unit/test_comparison_execution_record.py::test_a_cost_figure_cannot_be_carried_without_a_provenance` | Per-run costs (Gate-1 §4): USD 0.1780, 0.1709, 0.1827, 0.1702, 0.0551 for the five canonical runs;… | Provider revision unavailable | no |
| claim-10 | Data policy is committed and enforced | `src/techtree/models/data_policy.py` | `tests/unit/test_data_policy.py::test_public_report_against_a_private_policy_is_rejected` | Data policy sha256:6c532a43d595286a | Fixed v0.1 policy | no |
| claim-11 | Offline verification | `src/techtree/receipts/verify.py` | `tests/unit/test_local_bundle_verify.py::test_a_complete_proof_verifies_offline` | Gate-1 §4: 'proof verifies offline' for all five canonical runs; Gate-1 §8: independently re-confir… | No honest-compute proof | yes |
| claim-12 | Explicit approval before anything is spent | `src/techtree/runs/service.py` | `techtree-plugin/tests/unit/test_approvals.py::test_the_plugin_issues_no_approval_of_its_own` | Gate-1 §9: the native approval boundary sends nothing before consent — the approval surface is rend… | Surface attestation | yes |
| claim-13 | An incomplete comparison fails closed | `src/techtree/runs/real.py` | `tests/unit/test_episode_receipt_builder.py::test_a_missing_task_is_an_episode_count_mismatch` | Four failed runs preserved as failed with no Uplift receipt (Gate-1 §4 DIAGNOSTIC): run_a8da75d4 | No Uplift receipt | yes |
| scope-01 | Skill-bundle v1-vs-v2 comparison is supported when both bundles are supplied explicitly | `src/techtree/skills/scanner.py` | `tests/integration/test_multi_file_skill.py::test_the_draft_holds_the_whole_tree` | run_e29f178181bb4a65b0c518d34f72cd50 — the replacement run on derived campaign f2f04ae5..., report… | The certified replacement run compared two single-file bundles | yes |
| scope-02 | Guided revision is single-SKILL.md in v0.1; the flow is certified and no measured uplift is claimed | `src/techtree/uplift/source.py` | `tests/unit/test_verified_source_skill.py::test_the_entrypoint_text_comes_back_with_what_it_was_verified_against` | Gate-1 §7c: the single authorized reattempt at the predeclared 32,768 ceiling — response_id a2b2c61… | The flow is certified; the improvement is not | no |
| ext-01 | One pinned plugin installs and verifies Techtree | `techtree-plugin/bootstrap.py` | `techtree-plugin/tests/unit/test_bootstrap.py::test_a_missing_cli_produces_one_exact_plan` | ReleaseCore sha256:80807821d3d5b8bcf0a5223c96684b04546b250ff677b3e3bb65f768bde18687 — recomputed in… | placeholder_release is still true | yes |
| ext-02 | The result is shown deterministically and the explanation is guarded | `src/techtree/presentation/build.py` | `tests/unit/test_presentation_build.py::test_the_same_report_builds_the_same_bytes` | Gate-1 §7h: verified in the frozen build that the proof grade is read from the verified report (pre… | The explanation Skill is guarded, not verified | no |
| ext-03 | The proposed Skill v2 is shown and scanned before approval | `src/techtree/skills/scanner.py` | `tests/unit/test_skill_scanner.py::test_default_policy_matches_the_v01_instruction_skill_rules` | Gate-1 §7c: the proposal passed every guard and produced valid Skill v2 hello-world-v2; the sequenc… | Mechanical scan only | no |
| ext-04 | Nothing is uploaded to the website | `src/techtree/verifiers/config.py` | `tests/unit/test_verifiers_verify.py::test_a_resolved_config_that_would_upload_fails_the_push_check` | Gate-1 §9: no upload path observed — push=false in every resolved configuration across the certific… | The claim is about the Techtree website and the Verifiers platform, not about the network | yes |

## Gap register

Every cell below is an honest gap: the claim holds, but a specific artifact or a specific named test that the contract asks for does not exist yet. None of these was invented around, and none is fixed by this ticket.

1. **claim-05 — Clean subjects.** No trace-level artifact from any of the five canonical runs is committed to this repository. The only committed trace-level evidence is the sanitized recorded-evidence fixture from run_6ce5e56a7dd341bca8bc6de1d6a60027, which Gate-1 §4 classifies as DIAGNOSTIC, not canonical.
2. **claim-11 — Offline verification.** The contract asks for a command transcript as live evidence. No `techtree proof verify` transcript is committed in this repository: the run trees the five canonical bundles live in are outside the repo (runs/ is empty by design) and the terminal-journey transcript is an output of WP11e (docs/release/contracts/wp11e.md), which has not run. Current evidence is the Gate-1 §4/§8 attestation, not a citable transcript artifact.
3. **claim-12 — Explicit approval before anything is spent.** The approval audit events of the five canonical runs are in run journals under the participant's TECHTREE_HOME, not committed to this repository (runs/ is empty by design). The citable live evidence is the Gate-1 §7c/§9 attestation rather than the journal bytes.
4. **claim-13 — An incomplete comparison fails closed.** The four failed run records are preserved on the certification machine, not committed here. The citable evidence is the Gate-1 §4 DIAGNOSTIC listing and §3 immutability method, not the run bytes.
5. **scope-01 — Skill-bundle v1-vs-v2 comparison is supported when both bundles are supplied explicitly.** No live-evidence artifact exists for a multi-file bundle pair. Coverage for that case is tests/integration/test_multi_file_skill.py alone.
6. **ext-01 — One pinned plugin installs and verifies Techtree.** No fresh-install transcript is committed; that artifact is a WP11b/WP11e output, and neither ticket has run. Nothing here demonstrates the public install path end to end on a clean machine.
7. **ext-04 — Nothing is uploaded to the website.** WP11g (docs/release/contracts/wp11g.md) has not run, so the no-upload evidence today is the Gate-1 §9 observational attestation plus the tests above — not the three-method proof the release requires.

## Stop conditions

- *Any public claim with no implementation row* — not triggered. The authoritative v0.1 product statement (spec §18) was walked clause by clause; the four clauses not covered by the thirteen required rows became `ext-01`…`ext-04`.
- *Any row whose limitation contradicts public copy* — not triggered. Every limitation above is either already in public copy, already in a binding decision (0007 R5, 0013 §1, 0022, 0023 §4), or a deferral recorded against a ticket (ndq.3.42, ndq.3.43).

## Discrepancy noted, not fixed

`release/founder-skill-approval-draft.md` still opens with "Status: FINAL, awaiting the founder's Gate-1 approval phrase", but `release/founder-approvals/gate1-founder-skills.md` records that the approval phrase was received on 2026-08-14 against exactly this packet's digest (`sha256:b3ea3ba12af27af878fd4061f4a115b35f772b7c2ca61e2d4fb82d4b5701da35`, recomputed here from the working-tree bytes). The packet header is stale. Reported here, not amended — changing those bytes would change the approved digest.
